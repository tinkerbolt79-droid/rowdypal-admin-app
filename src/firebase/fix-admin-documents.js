  ...
  try {
    ...
    for (const docSnapshot of snapshot.docs) {
      ...
      if (
        ...
        console.log(
          `Would fix document ${docSnapshot.id} with email ${data.userId}`
        );
      }
      // Check if document ID doesn't match userId field
      else if (docSnapshot.id !== data.userId) {
        console.log(
          `Fixing admin document ID mismatch: ${docSnapshot.id} -> ${data.userId}`
        );
    ...
    console.log(
      `Admin document fix process completed. Fixed ${fixedCount} documents.`
    );
  } catch (error) {
